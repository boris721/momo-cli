...
export async function showStatus() {
  ...
  for (const log of timelogs) {
    ...
    if (log.typeId && log.type === 'project') {
      ...
      if (proj) {
        projectLabel = colorize(`[${proj.name}]`, proj.color);
      }
    }
    
    const desc = log.description ? `"${log.description}"` : '';
    const parts = [log.time, projectLabel, desc].filter(Boolean);
...
export async function addProject(name, options = {}) {
  ...
  // Normalize color to 8-char hex (#RRGGBBAA)
...
Commands:
  ...
  momo project add <name>         Create a new project
    --color #RRGGBB                 Set project color
